def first_task():
    ...
    # to True in order to satisfy 'different types' condition, so there are is no such entry
...
def second_task():
    ...
    zerg_units = ("mutalisk", "mutalisk", "mutalisk", "mutalisk", "mutslisk", "mutalisk", "mutalisk", "mutalisk",
...
def third_task():
    ...
    terran_units.union(starport_units)
    print(terran_units)
...
def eighth_task():
    ...
    for element in ("first", "second", "third"):
        sides.append(int(input("Enter the %s side size: " % element)))
    exists = True
    for side in sides:
        if side >= sum(set(sides).difference({side})):
            exists = False
    if not exists:
...
def tenth_task():
    ...
    print("Second way. May be not very different from the previous one. You may not score me point for that one :)")
    reversecomplement = ""
    for letter in DNA[::-1]:
...
for task in (first_task, second_task, third_task, fourth_task, fifth_task, sixth_task, seventh_task, eightth_task,
             ninegth_task, tenth_task, eleventh_task):
    task()
    print("\n\n")
